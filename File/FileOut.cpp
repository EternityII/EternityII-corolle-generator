...
FileOut::FileOut(int size, int corolle_size, int corolle_hamming, int posx, int posy, int piece_number, int rotation,
                 int coordonnees[25][3])
        : size(size), corolle_size(corolle_size), corolle_hamming(corolle_hamming), posx(posx), posy(posy),
...
void FileOut::open()
{
    ...
    if (fichier.good()) {
        ...
        << "_" << posx << "_" << posy
        << "_" << piece_number << "_" << rotation
        << ".txt" << endl;

        fichier << "# Fichier de resultat des Corolles possibles" << endl;

        fichier
        << "# [taille plateau] [taille de la corolle] [hamming] [coordonnees x] [coordonnees y] [id piece] [rotation]"
        << endl;

        fichier << size << " " << corolle_size << " " << corolle_hamming
        << " " << posx << " " << posy << " "
        << piece_number << " " << rotation << endl;
